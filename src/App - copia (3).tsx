...
const LiveTranslation = () => {
  ...
  const connectWebSocketAndStart = () => {
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        // ✨ ETAPA 2: Decodificar y reproducir audio Base64
        if (data.audio) {
          console.log("🎵 Datos de audio Base64 recibidos.");
          // El string Base64 se puede usar directamente en una URL de datos
        ...
        console.error("❌ Error al parsear o procesar el mensaje del WebSocket:", e);
  ...
  const playNextAudio = () => {
    if (audioQueueRef.current.length > 0) {
      ...
      audio.play().catch(e => console.error("❌ Error de reproducción, puede que no se haya activado el auto-play del navegador:", e));
    } else {
      console.log("⏹️ Cola de audio vacía. Deteniendo reproducción.");
